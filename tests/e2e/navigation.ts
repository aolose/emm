...
async function scenario5_AdminPages() {
	...
	for (const { path, title } of adminPages) {
		const { status, redirected, finalUrl } = await get(`${BASE}${path}`, {
...
async function scenario7_LogoutAndRedirect() {
	...
	assert(
		loginStatus === 200 || loginStatus === 307,
		'7.3 Login page accessible',
		`got ${loginStatus}`
	);
}
...
async function main() {
	...
	console.log(`\n${'═'.repeat(40)}`);
	console.log(
		`  Total: ${passed + failed}  |  \x1b[32mPassed: ${passed}\x1b[0m  |  \x1b[31mFailed: ${failed}\x1b[0m`
	);
