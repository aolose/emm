...
function assert(c: boolean, n: string, d?: string) {
	c ? ok(n) : fail(n, d);
}
...
async function scenarioE_FullFlowTrigger() {
	...
	assert(
		(r1.headers.get('location') || '').includes('/ts-challenge'),
		'E.2 redirects to challenge'
	);
...
async function main() {
	...
	try {
		await fetch(`${BASE}/api/statue`, { headers: HEADERS });
