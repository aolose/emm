...
function tableCols(table: string): string[] {
	try {
		return query(`PRAGMA table_info("${table}")`).map((r: any) => r.name);
	} catch {
		return [];
	}
}
...
for (const t of tables) {
	...
	try {
		onlineCols.push(...query(`PRAGMA online.table_info("${t}")`).map((r: any) => r.name));
	} catch {
		continue;
	}
	...
	if (missing.length) {
		...
		if (apply)
			for (const c of missing) db.run(`ALTER TABLE "${t}" ADD COLUMN "${c}" TEXT DEFAULT ''`);
	}
}
...
log('\n--- Step 1: Fix r2Key ---');
const badR2 = query(
	`SELECT id, md5, r2Key FROM Res WHERE md5 IS NOT NULL AND md5 != '-' AND r2Key != substr(md5,1,6)`
);
...
run(
	"UPDATE Res SET r2Key = substr(md5,1,6) WHERE md5 IS NOT NULL AND md5 != '-' AND r2Key != substr(md5,1,6)",
	'Fixing...'
);
...
const sets = syncCols
	.map((c) => `"${c}" = (SELECT "${c}" FROM online.System WHERE id=1)`)
	.join(',\n    ');
...
const mergeTables: Record<string, string> = {
	...
	PostTag:
		'INSERT INTO main.PostTag   SELECT * FROM online.PostTag   WHERE (postId||":"||tagId) NOT IN (SELECT postId||":"||tagId FROM main.PostTag)',
	Comment:
		'INSERT INTO main.Comment   SELECT * FROM online.Comment   WHERE id NOT IN (SELECT id FROM main.Comment)',
	CmUser:
		'INSERT INTO main.CmUser    SELECT * FROM online.CmUser    WHERE id NOT IN (SELECT id FROM main.CmUser)',
	Require:
		'INSERT INTO main.Require   SELECT * FROM online.Require   WHERE id NOT IN (SELECT id FROM main.Require)',
	RequireMap:
		'INSERT INTO main.RequireMap SELECT * FROM online.RequireMap WHERE id NOT IN (SELECT id FROM main.RequireMap)'
};
