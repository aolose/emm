...
export class Client {
  constructor(skip?: boolean, uuid?: string) {
    ...
    if (!skip) clientMap.set((this.uuid = uuid || randomUUID()), this);
  }
  ...
  has(tk: { type: permission; _reqs?: number[] } | Obj<TokenInfo>) {
    ...
    if (t) {
      if (t === permission.Admin || t === permission.Read) {
        return true;
      }
      if (_reqs) {
  ...
  addToken(tk: TokenInfo, skip = false) {
    ...
    if (type == permission.Read || type === permission.Admin) this.tokens.set(type, expire || -1);
  ...
  ok(p: permission) {
    this.clear();
    if (debugMode || this.tokens.get(permission.Admin)) return true;
