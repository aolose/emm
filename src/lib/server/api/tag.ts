...
const apis: APIRoutes = {
	tagLS: {
		post: auth(Read, async () => {
			...
			return ts.map((t) => {
				...
				if (ps.length) {
					return { ...t, _posts: db.all(model(Post), `id in (${sqlFields(ps.length)})`, ...ps).map((a) => ({ id: a.id, title: a.title || a.title })) };
				}
	...
	tag: {
		post: auth(Admin, async (req) => {
			...
			if (tag.id) { t = ts.find((a) => a.id === tag.id); }
			const { _posts } = tag;
			try {
				if (t) { await throwDbProxyError(Object.assign(t, filter(tag, ['banner','desc','name'], false))); }
				else ts.unshift(await throwDbProxyError((t = DBProxy(Tag, tag))));
			} catch (e) {
				if (e instanceof Error) { let msg = e.message; if (msg.includes('UNIQUE constraint')) msg = 'tag name exist'; return resp(msg, 500); }
			}
		...
		delete: auth(Admin, async (req) => {
			...
			if (t) { tagPostCache.delete([], t.id); db.del(t); tags.update((tt) => tt.filter((a) => a.id !== id)); }
		})
	},
	tags: {
		get: async (req) => { return getPubTags(getClient(req)).map((a) => a.name).join(); },
