...
const hitRule = (r: {
  ...
  if (k.status && k.status !== status) return false;
  if (k.method) {
    const ms = k.method
      .toLowerCase()
      .split(",")
...
export function loadRules() {
  ...
  db.all(model(FWRule)).forEach(a=>{
	...
	blackList=db.all(model(BlackList))
	blackList.forEach(a => {
    rules.push(a.toRule());
  });
  sort();
...
export const delBlackList = (id:number)=>{
	...
	blackList=blackList.filter(a=>a.id!==id)
	rules=rules.filter(r=>r.id!==-id)
}

export const blackLists = (page=1,size=20)=>{
    const bs =  blackList.slice(size * (page - 1), size * page)
    return {
...
const logToReq = (l: log) => {
  ...
  return {
    createAt, ip, path, headers: new Headers(str2Hds(headers)), status, method
  };
};
