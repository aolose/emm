...
export const ruv = ({ ip, status, path, ua }: { ip: string, status: number, path: string, ua: string }) => {
  const valid = status < 300 && (path === "/" || /^\/((post|tag|)s?|ticket)/.test(path)) && !isbot(ua);
  rqD++;
  rqH++;
  if (valid) {
    pvD++;
...
export const getRuv = (start: number, end: number, type = 0) => {
  return arrFilter(db.all(model(RPU), `t>=? and t<=? and type=? order by t asc`, start, end, type), [
    "t", "r", "p", "u", "ur"
  ]);
};