...
const apis: APIRoutes = {
	...
	genCode: {
		post: auth(Admin, async (req) => {
			const { expire, type, times, reqs, share } = await req.json();
	...
	backup: {
		post: async (req) => {
			if (sysStatue && !getClient(req)?.ok(Admin)) return resp('no permission', 403);
			...
			server.stop();
			try {
				fs.writeFileSync(cfgPath, dbPath, { flag: 'w' });
